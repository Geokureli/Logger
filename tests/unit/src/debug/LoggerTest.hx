...
class LoggerTest extends utest.Test
{
    ...
    function testColorSanitize()
    {
        UAssert.equals(new Logger(StyleTools.color(log1.id, RED)), log1);
        
        final sub1 = log1.sub("a");
        final sub2 = log1.sub(StyleTools.color("a", RED));
        UAssert.equals(sub1, sub2);
        
        final context1 = new Logger(log1.id + "[context]");
        final context2 = new Logger(log1.id + StyleTools.color("[context]", RED));
